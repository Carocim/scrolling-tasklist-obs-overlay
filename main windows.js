...
// Enhanced command line switches for better OBS compatibility
...
// Additional switches for Windows OBS compatibility
...
function createOverlayWindow() {
  // Create with better OBS compatibility settings
  overlayWindow = new BrowserWindow({
    ...
    transparent: true,
    alwaysOnTop: false,
    skipTaskbar: false, // Changed to false for better OBS detection
    resizable: false,
    hasShadow: false,
    backgroundColor: '#00000000', // Proper transparent background
    show: false, // Don't show immediately
    webPreferences: {
      nodeIntegration: true,
      contextIsolation: false,
      backgroundThrottling: false,
      offscreen: false // Ensure it's not offscreen rendered
    }
  });
  ...
  overlayWindow.once('ready-to-show', () => {
    ...
    // Force window to be visible to capture systems
    overlayWindow.showInactive();
    overlayWindow.focus();
    overlayWindow.blur(); // Remove focus but keep visible
  });
  ...
  if (process.platform === 'win32') {
    ...
    // Force window to be in the window list
    overlayWindow.once('show', () => {
      setTimeout(() => {
        overlayWindow.setTitle('Task Overlay - OBS Capture');
        overlayWindow.setSkipTaskbar(false);
      }, 100);
    });
  }
...
// Additional IPC handler for OBS mode
ipcMain.handle('set-obs-mode', (event, enabled) => {
  if (overlayWindow) {
    if (enabled) {
      // OBS-friendly mode
      overlayWindow.setAlwaysOnTop(false);
      overlayWindow.setSkipTaskbar(false);
      overlayWindow.setTitle('Task Overlay - OBS Capture');
      overlayWindow.webContents.send('obs-mode-changed', true);
    } else {
      // Normal overlay mode
      overlayWindow.setAlwaysOnTop(true);
      overlayWindow.setSkipTaskbar(true);
      overlayWindow.setTitle('Task Overlay');
      overlayWindow.webContents.send('obs-mode-changed', false);
    }
  }
});